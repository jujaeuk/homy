...
while(@$check=mysqli_fetch_object($result)){
  fwrite($fp,"no. ".$check->no." ".html_entity_decode($check->title)." (".$check->writer.", ".date("Y-m-d H:i",$check->time).")\n");
  ...
  if($check->upper!=0){
    ...
    fwrite($fp,"upper: no.".$check_upper->no." ".html_entity_decode($check_upper->title)."\n");
  }
  ...
  if(mysqli_num_rows($result_lower)>0){
    ...
    while($check_lower=mysqli_fetch_object($result_lower)){
      fwrite($fp,"no .".$check_lower->no." ".html_entity_decode($check_lower->title)."\n");
    }
  }
  fwrite($fp,"\n");
}
